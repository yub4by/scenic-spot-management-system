  ...
  <form name="myForm" id="myForm" action="SpotServlet?flag=alldel" method="post">
	...
	<table width="99%" border="0" cellspacing="0" cellpadding="0" id="searchmain">
	  <tr>
	    ...
	    <div class="place"><span>您的位置：景点管理 > 景点列表</span></div>
	    </td>
	  </tr>
	  <tr>
		    ...
		    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search">
		  		<tr>
			        ...
			        <div>
					         ...
					         景区：<input type="text" name="address" id="address" value="${param.address }" class="text-word">
	    ...
	    <td align="left" valign="top">
		    <table width="100%" border="0" cellspacing="0" cellpadding="0" id="main-tab">
		      <tr>
		        ...
		        <th align="center" valign="middle" class="borderright">景点ID</th>
		      ...
		      <c:forEach var="spot" items="${spotList }">
    			<tr onMouseOut="this.style.backgroundColor='#ffffff'" onMouseOver="this.style.backgroundColor='#edf5ff'">
    				...
    				<td align="center" valign="middle" class="borderbottom">
	    				...
	    				&nbsp;|&nbsp;
	    				<a href="SpotServlet?flag=del&id=${spot.id }" onclick="return confirm('确定删除吗？')" target="mainFrame" onFocus="this.blur()" class="add" >
	    	...
	    	<div>
	    		...
	    		<c:choose>
	    			<c:when test="${page.hasPre }">
	    				<a href="SpotServlet?flag=${list }&pageIndex=1" target="mainFrame" onFocus="this.blur()">首页</a>
	    				<a href="SpotServlet?flag=${list }&pageIndex=${page.pageIndex-1 }" target="mainFrame" onFocus="this.blur()">上一页</a>
	    			</c:when>
	    			<c:otherwise>
	    				首页	上一页
	    			</c:otherwise>
	    		...
	    		<c:choose>
	    			<c:when test="${page.hasNext }">
	    				<a href="SpotServlet?flag=${list }&pageIndex=${page.pageIndex+1 }" target="mainFrame" onFocus="this.blur()">下一页</a>
	    				<a href="SpotServlet?flag=${list }&pageIndex=${page.pageCount }" target="mainFrame" onFocus="this.blur()">尾页</a>
	    			</c:when>
	    			<c:otherwise>
	    				下一页	尾页
	    			</c:otherwise>
